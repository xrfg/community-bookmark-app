import React from "react";
import "../Sass/Description.scss";

export default function Description() {
  return (
    <div className="description">
      <div>
        <h2>bookmark collective</h2>
        <p>
          {" "}
          like big cats and i can not lie head nudges . Throw down all the stuff
          in the kitchen refuse to come home when humans are going to bed; stay
          out all night then yowl like i am dying at 4am yet mess up all the
          toilet paper so stare at the wall, play with food and get confused by
          dust but toilet paper attack claws fluff everywhere meow miao french
          ciao litterbox, catch mouse and gave it as a present so with tail in
          the air.
        </p>
      </div>
    </div>
  );
}
